...
function Unit:update(dt) 
	...
	local sx, sy = love.math.random(-100, 100) * dt, love.math.random(-100, 100) * dt
	self.x = self.x + sx
	self.y = self.y + sy
	if not self:isOnScreen() or self:collideWithAny() then
		self.x = oldx
		self.y = oldy
	elseif not self:isOnScreen() then
		self:fixCollidedXY()		
	end	
end
...
function Unit:collideWith(other) 
	if self == other then
		return false
	end
...
function Unit:fixCollidedXY()
	while self:collideWithAny() do
		self.x = love.math.random(20,love.graphics.getWidth()-20)
		self.y = love.math.random(20,love.graphics.getHeight()-20)
	end
end
